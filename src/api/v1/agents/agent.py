...
def validate_node(state: RAGState) -> RAGState:
    ...
    if not docs:
        # BUG FIX: must check max_retries here too — otherwise the graph loops
        # forever because validation_passed stays False and route_validation
        # keeps sending execution back to retriever_node indefinitely.
        if retry_count >= max_retries:
            print(f"[validate_node] No docs after {max_retries} retries — forcing PASS to exit loop.")
            return {
                **state,
                "validation_passed": True,   # forces route → generate_answer → END
...
def generate_answer_node(state: RAGState) -> RAGState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            "system",
            "You are a helpful compliance assistant. Answer the user's question using ONLY the "
